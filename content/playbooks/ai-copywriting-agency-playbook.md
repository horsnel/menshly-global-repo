---
title: "The AI Copywriting Agency Playbook: 44 Steps to $20K/Month"
date: 2026-05-01
category: "Playbook"
price: "₦25,000"
readTime: "75 MIN"
excerpt: "The complete operating system for building an AI copywriting agency from zero. 10 modules, 32 procedures, exact tool configurations, client acquisition scripts, three pricing tiers, and a scaling roadmap. From blank page to ₦8M/month in recurring revenue."
image: "/images/articles/playbooks/ai-copywriting-agency-playbook.png"
heroImage: "/images/heroes/playbooks/ai-copywriting-agency-playbook.png"
...
- [ ] Professional domain and email configured
- [ ] Service Menu Document created with all five service rows
- [ ] Price floor set at minimum ₦150,000/month
...
4. Install the Grammarly browser extension. This gives you real-time checking inside ChatGPT, Google Docs, Notion, and your email.
5. Install the Grammarly desktop app for offline document checking.
...
3. Create the Client Template with these properties:
...
3. Set up [Wise](https://wise.com/) for international clients.
4. Create an invoice template with:
...
3. Create a "Test Results" sub-page where you log the output quality of each prompt on a 1-5 scale after first use.
4. Any prompt scoring below 3 must be revised before client use.
...
- [ ] Each prompt tested at least once with sample client data
- [ ] Test Results sub-page created with quality scores
- [ ] Any prompt scoring below 3 revised and retested
- [ ] Prompt Library index table complete with all metadata
...
1. **Product/Service Name** — What exactly are we writing about?
2. **Offer** — What is the reader being asked to do?
3. **Price** — How much does it cost? In Naira.
4. **Target Audience** — Demographics, psychographics, current situation
5. **Pain Point** — What specific problem does the audience have?
6. **Desired Outcome** — What does the audience want?
7. **Objections** — Why haven't they bought already? (List top 3)
8. **Competitors** — Who else offers this? What makes this different?
9. **Proof Elements** — Testimonials, case studies, data, certifications
10. **Tone** — 3 adjectives describing the brand voice
11. **Deadline** — Exact date and time (WAT)
12. **Format** — Which service from the Service Menu?
...
3. Fill in all bracketed variables with data from the brief and research phase.
4. Execute the prompt. Read the full output before proceeding.
5. Run the output through this revision prompt:
...
6. Apply all suggested improvements from the revision prompt.
...
1. Sales page or landing page first — this establishes the core messaging and value propositions
2. Email sequence second — it extends the sales page messaging
3. Ad copy third — it distills the messaging into shortest form
4. Social media last — it repurposes all of the above into platform-native formats
...
- [ ] Voice File template created in client folder structure
- [ ] Revision prompt tested and producing actionable feedback
- [ ] Production batch order (Sales → Email → Ads → Social) locked as SOP
- [ ] Internal and client-facing timelines documented and posted
...
1. Paste the draft into the Grammarly editor or open it in a Google Doc with the Grammarly extension active.
2. Resolve ALL red (correctness) suggestions. No exceptions.
3. Review ALL yellow (clarity) suggestions. Accept if they improve readability without changing meaning. Reject if they flatten the voice.
4. Review ALL blue (engagement) suggestions. These are optional. Accept only if they genuinely strengthen the copy.
5. Review ALL green (delivery) suggestions. Check formality level. Adjust if the copy feels too stiff or too casual for the client's brand.
6. The Grammarly overall score must be 85 or above before proceeding. If it is below 85, revise and recheck.

**Pass 2: Brand Voice Audit**
1. Open the client's Voice File from their Notion folder.
2. Read the draft aloud. Mark any sentence that sounds like it could belong to any brand (generic).
3. Check against the client's vocabulary list: Are the 10 frequent words/phrases used naturally?
4. Check against the forbidden list: Are any of the 5 forbidden words/phrases present?
5. Compare the draft against the 3 example copy pieces the client loves. Does the draft match the energy, rhythm, and vocabulary?
6. Revise any generic sentences with client-specific language, examples, or references.

**Pass 3: Conversion Logic Check**
1. Read only the first line of every section. Does the narrative flow logically? Hook → Problem → Solution → Proof → CTA?
2. Count the CTAs. Is there exactly one primary CTA per piece? (Exception: long-form sales pages can have 3 CTA placements, but they must all direct to the same action.)
3. Check for specificity: Highlight every vague claim ("save time," "get better results," "grow your business"). Replace each with a specific claim ("save 4 hours per week," "increase conversions by 23%," "add ₦500K in monthly revenue").
...
1. **Remove AI fingerprints.** Search for and delete these common AI-isms:
...
2. **Add human imperfection.** Insert:
...
- [ ] GPTZero or Originality.ai account active and tested
- [ ] QC Checklist template created in Notion
- [ ] Grammarly minimum score threshold (85) enforced on all deliverables
- [ ] Zero deliverables shipped without PASS status
...
1. Create a Google Drive folder: `[Agency Name] → [Client Name] → [Project Name] → [Date]`
2. Organize files in sub-folders by format:
   - `/01-Sales-Page/`
   - `/02-Email-Sequence/`
   - `/03-Ad-Copy/`
   - `/04-Social-Media/`
   - `/05-SMS-WhatsApp/`
3. Include a "Content Map" document showing how each piece connects:
   ```
   SALES PAGE
   ...
   ...
   ```
4. Deliver via the Make.com automation from Procedure 2.4.
...
- [ ] Format adaptation rules table saved in SOPs
- [ ] Delivery packaging structure created in Google Drive
- [ ] Content Map template created and included in at least one delivery
- [ ] Repurposing output tracked: minimum 8 derivative assets per long-form piece
...
8. List 3 examples of copy you love (URLs or screenshots)
9. List words or phrases we should NEVER use
10. What is the primary call to action for this project?
11. Any current promotions, deadlines, or launches we should know about?
...
1. After payment is confirmed in Paystack, trigger:
...
- [ ] Brand Brief form created in Typeform or Google Forms
- [ ] Onboarding sequence documented as SOP
- [ ] Make.com automation updated with all three trigger scenarios
- [ ] First test client onboarded through the complete 7-step sequence
...
- [ ] Proposal template created in Notion and tested on one prospect
- [ ] Negotiation framework rehearsed with responses to 4 common objections
- [ ] Revenue model calculated with Month 6 target
- [ ] "Never discount, reduce scope" rule enforced as agency policy
...
**MBR Agenda:**
1. **Results review (10 min):** What copy shipped? What performed? Pull any available metrics (email open rates, ad CTR, social engagement). If the client doesn't share data, ask: "Can you share last month's email open rates and ad click-through rates? I want to optimize next month's copy based on what's working."
2. **Content preview (10 min):** Show next month's content calendar. Get approval on topics and angles.
3. **Upsell conversation (5 min):** "Based on what's performing, I'd recommend adding [additional service]. We could [specific outcome]. Want me to put together a proposal?"
4. **Feedback (5 min):** "On a scale of 1-5, how satisfied are you with our work this month? What would make it a 5?"
...
1. **After 3 successful months:** Propose adding a new content type (e.g., "Your social posts are performing well. Let's add ad copy to amplify your top-performing content.")
2. **After a specific win:** "That email sequence generated [result]? Imagine what a dedicated sequence for [new product/audience] could do. Want me to draft one?"
3. **Quarterly:** Offer a "Brand Voice Refresh" — update the Voice Guide, review competitor shifts, and optimize the prompt library for the client. Charge ₦100,000-₦200,000 as a one-off or fold it into the Enterprise tier.
...
- [ ] MBR agenda template saved in Notion
- [ ] Churn prevention protocol with 4 early warning signals documented
- [ ] Expansion revenue triggers defined and calendar reminders set
- [ ] Client Anniversary system created and scheduled for first 3 clients
...
1. **Every deliverable passes through a Senior Editor** (you or your Copy Editor) before client delivery. No exceptions. Junior copywriters produce drafts. Editors approve them. This is non-negotiable until a copywriter has produced 50+ deliverables with a 95% first-pass approval rate.

2. **Weekly Prompt Library Review.** Every Friday, review the week's outputs. Identify any prompt that produced subpar results. Revise the prompt. Update the Notion library. Notify the team.

3. **Monthly Calibration Session.** Pick 5 random deliverables from the past month. Review them as a team against the QC Checklist. Discuss discrepancies. Align standards.
...
4. **Case study content (monthly):** After every client win, write a 500-word case study. Post it on LinkedIn, your website, and include it in proposals. Specificity wins: "How we increased email open rates by 34% for a Lagos fintech in 30 days."
...
- [ ] Quality maintenance systems (editor review, prompt review, calibration) documented
- [ ] All 12 SOPs written and stored in Notion
- [ ] Financial operations system set up (business account, P&L template, tax reserve)
- [ ] Growth marketing plan active: LinkedIn daily, referrals launched, cold outreach weekly
- [ ] Free homepage rewrite offer tested and conversion rate tracked
...
- [ ] Agency Identity Document complete with ICP, positioning, and service menu
- [ ] ChatGPT Custom GPT configured with base instructions and data training off
- [ ] Grammarly Business configured with Style Guide and client-specific rules
- [ ] Notion workspace built with all 6 top-level pages and client template
- [ ] Make.com running 3 core automations (brief intake, delivery, revenue report)
- [ ] Paystack payment infrastructure active with invoice template
- [ ] Prompt Library complete with 9+ prompts (A through I minimum) tested and scored
- [ ] Three-pass QC system enforced on every deliverable
- [ ] Anti-AI detection protocol tested (below 30% on GPTZero/Originality.ai)
- [ ] Repurposing engine producing 8+ derivative assets per long-form piece
- [ ] 7-step onboarding sequence documented and automated
- [ ] Three-tier pricing table (Starter ₦500K / Growth ₦1.2M / Enterprise ₦2.5M) finalized
- [ ] Proposal template tested on at least one prospect
- [ ] Weekly delivery rhythm (Mon-Fri cadence) operational
- [ ] Monthly Business Review agenda ready
- [ ] Churn prevention protocol with 4 early warning signals active
- [ ] Hiring roadmap with salary benchmarks ready
- [ ] All 12 SOPs written in Notion
- [ ] Business bank account separated from personal
- [ ] Growth marketing plan: LinkedIn + referrals + cold outreach active
